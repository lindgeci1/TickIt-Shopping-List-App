...
const CARD_WIDTH = (width - 20) * 0.45; // 2 per row
const Separator = ({ widthPercent = 80, thickness = 1, marginVertical = 6, color = "#e0e0e0" }) => (
...
export default function ProductCardTOBrowse({
  ...
  useEffect(() => {
    Animated.spring(scale, {
      toValue: selected ? 0.95 : 1,
      friction: 7,
      useNativeDriver: true,
    }).start();

    Animated.timing(overlayOpacity, {
      toValue: selected ? 0.25 : 0,
      duration: 200,
      useNativeDriver: true,
    }).start();
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Checkmark in center */}
        {selectionMode && (
          <Animated.View style={[styles.centerCheck, { transform: [{ scale: checkScale }] }]}>
            <Text style={styles.checkMark}>✓</Text>
          </Animated.View>
        )}

        <Image
          source={{ uri: product.Photos?.[0] || "https://via.placeholder.com/100" }}
          style={styles.photo}
          resizeMode="cover"
        />
...
centerCheck: {
  ...
  shadowOpacity: 0.3,
  shadowRadius: 2,
  transform: [{ translateX: -16 }, { translateY: -16 }], // perfectly center
},
  ...
  category: { fontSize: 12, color: "#888", textAlign: "center", marginBottom: 4 },
  ...
  divider: { width: "80%", height: 1, backgroundColor: "#e0e0e0", marginVertical: 6 },
  thinDivider: { width: "60%", height: 0.5, backgroundColor: "#e0e0e0", marginVertical: 3 },
 preferredBox: {
