...
import { filterShoppingLists } from "./filterShoppingLists";

export default function ShoppingListScreen({ visible, onClose, shoppingLists, onSelect, onCreate }) {
  // Use separate hooks
  const {
    newListName,
    setNewListName,
    loading: loadingAdd,
    message: messageAdd,
    messageColor: messageColorAdd,
    highlightId,
    handleCreate
  } = addShoppingList(onCreate);
  ...
  const {
    ...
    messageColor: messageColorSelect,
    handleSelect
  } = filterShoppingLists({ visible, onSelect });

  const loading = loadingAdd || loadingSelect;
  const message = messageAdd || messageSelect;
  const messageColor = messageAdd ? messageColorAdd : messageColorSelect;
        ...
        <View style={styles.container}>
              ...
              renderItem={({ item }) =>
                item && (
                  <TouchableOpacity
                    style={[
                      styles.listItem,
                      item.Shopping_List_ItemID === selectedId && styles.selectedListItem,
          ...
          <View style={styles.createContainer}>
            ...
            <TouchableOpacity
              ...
              onPress={newListName ? handleCreate : null}
              disabled={loading}
            >
              {loading ? (
          ...
          {message.length > 0 && (
            <View
              style={[
                ...
                { backgroundColor: messageColor === "#155724" ? "#d4edda" : messageColor === "#856404" ? "#fff3cd" : "#f8d7da" }
              ]}
            >
