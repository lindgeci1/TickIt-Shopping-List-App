...
export default function ShoppingLists({ item, index, onDelete, onUpdateProducts, listName, onProductAdded }) {
  const [expanded, setExpanded] = useState(false), [products, setProducts] = useState([]), [showToBuy, setShowToBuy] = useState(false), [showBought, setShowBought] = useState(false), [totalBoughtPrice, setTotalBoughtPrice] = useState(0), [retrieveMode, setRetrieveMode] = useState(false), [modalVisible, setModalVisible] = useState(false), [toBuySelectionMode, setToBuySelectionMode] = useState(false), [toBuyEditMode, setToBuyEditMode] = useState(false), [toBuySelectedProducts, setToBuySelectedProducts] = useState([]), [boughtSelectionMode, setBoughtSelectionMode] = useState(false), [boughtSelectedProducts, setBoughtSelectedProducts] = useState([]);
  ...
  const handleUpdateSelectedToBuy = async () => {
    ...
    if (productsWithoutMarket.length > 0) {
      setToastMessage(`Attach a market before buying: ${productsWithoutMarket.join(", ")}`);
    } else {
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { width: "85%", padding: 20 }]}>
            ...
            <View style={{ flexDirection: "row", justifyContent: "flex-end", gap: 10 }}>
              <TouchableOpacity onPress={() => setAddProductModalVisible(false)} style={{ paddingVertical: 10, paddingHorizontal: 15, borderRadius: 8, backgroundColor: "#fff", borderWidth: 1, borderColor: "#6c63ff" }}>
                <Text style={{ color: "#6c63ff", fontWeight: "600" }}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={handleAddProduct} style={{ backgroundColor: "#6c63ff", paddingVertical: 10, paddingHorizontal: 15, borderRadius: 8 }}>
            ...
            {showToBuy && (
              <View style={{ height: toBuyProducts.length ? Math.min(toBuyContentHeight + 80, screenHeight * 0.25) : screenHeight * 0.06 }}>
                {toBuyProducts.length ? (
                  <>
                    <ScrollView style={{ flex: 1 }} onContentSizeChange={(width, height) => { const roundedHeight = Math.round(height); if (Math.abs(roundedHeight - toBuyContentHeight) > 1) { setToBuyContentHeight(roundedHeight); } }}>
                    ...
                    {!toBuySelectionMode ? (
                      <View style={styles.actionsRow}>
                        <TouchableOpacity style={[styles.actionButton, { backgroundColor: toBuyProducts.length ? "#6c63ff" : "#ccc" }]} disabled={!toBuyProducts.length} onPress={() => { setToBuySelectionMode(true); setToBuyEditMode(false); }}>
                          <Text style={styles.actionText}>Select to remove</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={[styles.actionButton, { backgroundColor: toBuyProducts.length ? "#ffa500" : "#ccc" }]} disabled={!toBuyProducts.length} onPress={() => { setToBuySelectionMode(true); setToBuyEditMode(true); }}>
                          <Text style={styles.actionText}>Select To Buy</Text>
                        </TouchableOpacity>
                      </View>
                    ) : (
                      <View style={styles.actionsRow}>
                        <TouchableOpacity style={[styles.actionButton, { backgroundColor: toBuyEditMode ? "#6c63ff" : "#ff6b6b" }]} onPress={toBuyEditMode ? handleUpdateSelectedToBuy : () => handleRemoveSelected(toBuySelectedProducts, "ToBuy")}>
                          <Text style={styles.actionText}>Confirm</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={[styles.actionButton, { backgroundColor: "#aaa" }]} onPress={() => { setToBuySelectionMode(false); setToBuyEditMode(false); setToBuySelectedProducts([]); }}>
                  ...
                  <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
                    <Text style={{ fontStyle: "italic", color: "#666" }}>No products in the to-buy list</Text>
                  </View>
                )}
              </View>
            )}
            ...
            {showBought && (
              <View style={{ height: boughtProducts.length ? Math.min(boughtContentHeight + 80, screenHeight * 0.25) : screenHeight * 0.06 }}>
                {boughtProducts.length ? (
                  <>
                    <ScrollView style={{ flex: 1 }} onContentSizeChange={(width, height) => { const rounded = Math.round(height); if (Math.abs(rounded - boughtContentHeight) > 1) { setBoughtContentHeight(rounded); } }}>
                    ...
                    {!boughtSelectionMode ? (
                      <View style={styles.actionsRow}>
                        <TouchableOpacity style={[styles.actionButton, { backgroundColor: boughtProducts.length ? "#6c63ff" : "#ccc" }]} disabled={!boughtProducts.length} onPress={() => { setBoughtSelectionMode(true); setRetrieveMode(false); }}>
                          <Text style={styles.actionText}>Select to remove</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={[styles.actionButton, { backgroundColor: boughtProducts.length ? "#ffa500" : "#ccc" }]} disabled={!boughtProducts.length} onPress={() => { setBoughtSelectionMode(true); setRetrieveMode(true); }}>
                          <Text style={styles.actionText}>Select to retrieve</Text>
                        </TouchableOpacity>
                      </View>
                    ) : (
                      <View style={styles.actionsRow}>
                        <TouchableOpacity style={[styles.actionButton, { backgroundColor: retrieveMode ? "#6c63ff" : "#ff6b6b" }]} onPress={retrieveMode ? handleRetrieveSelectedBought : () => handleRemoveSelected(boughtSelectedProducts, "Bought")}>
                          <Text style={styles.actionText}>Confirm</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={[styles.actionButton, { backgroundColor: "#aaa" }]} onPress={() => { setBoughtSelectionMode(false); setBoughtSelectedProducts([]); setRetrieveMode(false); }}>
                          <Text style={styles.actionText}>Cancel</Text>
                        </TouchableOpacity>
                      </View>
                    )}
                  ...
                  <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
                    <Text style={{ fontStyle: "italic", color: "#666" }}>No products in the bought list</Text>
                  </View>
                )}
              </View>
            )}
